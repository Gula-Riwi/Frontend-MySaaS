            ...
            <div class="flex-1 overflow-y-auto p-8 z-10 scrollbar-hide">
                ...
                <div class="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p class="text-gray-400">Analizando {{ project?.name || 'Proyecto' }}</p>
                    </div>
                    <button @click="sendSmartReport" 
                        class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors">
                        <span>📧 Enviar Reporte Inteligente</span>
                    </button>
                </div>
                    ...
                    <div class="bg-gray-900 border border-indigo-500/30 rounded-2xl p-6 shadow-lg shadow-indigo-500/10">
                        ...
                        <div class="space-y-4">
                            ...
                            <div v-else class="p-4 bg-green-500/10 border border-green-500/20 rounded-xl">
                                ...
                                <p class="text-gray-300 text-sm">{{ insights.staffing?.message || 'Nivel de personal adecuado.' }}</p>
                            </div>
                            ...
                            <div class="p-4 bg-purple-500/10 border border-purple-500/20 rounded-xl">
                                ...
                                <p class="text-gray-300 text-sm">{{ insights.optimization?.suggestion || 'Analizando mejor horario...' }}</p> 
                                <p class="text-gray-500 text-xs mt-1" v-if="insights.optimization?.bestTime">Mejor hora: {{ insights.optimization?.bestTime }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-gray-900 border border-pink-500/30 rounded-2xl p-6 shadow-lg shadow-pink-500/10">
                        ...
                        <div class="space-y-4">
                             ...
                             <div class="p-4 bg-gray-800/50 rounded-xl">
                                ...
                                <p class="text-gray-500 text-xs">{{ insights.revenue?.suggestion || 'Sin sugerencias por ahora.' }}</p>
                             </div>
...
const sendSmartReport = async () => {
    if (!project.value?.id) return;
    try {
        await aiInsightsService.sendSmartAlertsReport(project.value.id);
        alert('Reporte enviado a tu correo exitosamente.');
    } catch (e) {
        console.error(e);
...
onMounted(async () => {
    ...
    if (currentProject) {
        ...
        try {
            ...
            if (resp?.data?.success) {
                const projects = resp.data.data.projects || resp.data.data;
    ...
    if (projectId) {
        ...
        try {
            // Note: dashboardService.getStats and getTasks might fail if endpoints don't exist yet, 
            // but we assume they do from the remote branch. 
            // If they fail, we wrap in individual try/catch or allow partial loading? 
            // For now, doing Promise.allSettled might be safer, but let's stick to simple Promise.all for consistency.
            
            // Wait, we need to extract data correctly.
            // dashboardService likely returns data directly (as per service definition).
            // aiInsightsService now returns data directly.
            
            const [stats, tasks, aiData] = await Promise.all([
                dashboardService.getStats(projectId).catch(() => ({ monthlySales: {value:0}, aiConversations:{value:0}, automatedOrders:{value:0}, hoursSaved:{value:0} })),
                dashboardService.getTasks(projectId, { period: 'month' }).catch(() => []),
                aiInsightsService.getProjectInsights(projectId).then(res => res.data).catch(() => null)
            ]);

            // 3. Update Metrics (Stats)
            if (stats) {
            ...
            // 5. Update Insights
            // aiData comes from aiInsightsService.getProjectInsights which we defined to return response.data originally.
            // but in the service file I wrote: return response.data.
            // Dashboard expects { data: ... } or just the payload?
            // The API usually returns { success: true, data: { ... } }
            // So response.data in service = { success: true, data: ... }
            // So aiData = { success: true, data: ... }
            if (aiData && aiData.success) {
                insights.value = aiData.data;
            }
