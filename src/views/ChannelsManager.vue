                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-400 truncate max-w-[150px]" title="Ver detalles">{{ getChannelValue(channel) }}</p>
                                        </div>
                                    </div>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" @click.self="showModal = false">
            <div class="bg-gray-900 rounded-2xl w-full max-w-md border border-white/10 shadow-2xl overflow-hidden">
                ...
                <div class="p-6 border-t border-white/10 bg-gray-900/50 flex justify-end gap-3">
                    ...
                    <button @click="saveChannel" :disabled="saving" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-xl font-bold transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
...
const openModal = (channel = null) => {
    if (channel) {
        ...
        form.value.type = channel.type; // Note: if custom types exist, logic might need check
